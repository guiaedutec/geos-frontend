...
class MenuBurger extends React.Component {
  ...
  render() {
    const { user } = this.props;
    return (
      <div className={classnames("is-hidden-desktop")}>
        {/* Seção de escolas */}
        {(isAdminStateCity(this.getUserProfile()) ||
          isAdminCountry(this.getUserProfile())) && (
          <div
            className={classnames(
              "navbar-item has-text-weight-bold is-hidden-touch",
...
export default injectIntl(compose(AccountsContainer)(MenuBurger));
